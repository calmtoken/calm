...
contract ACALM is Context, IERC20, Ownable {
  ...
  function setDevAddress(address newDev) public onlyOwner() {
    dev_address = newDev;
  }
  ...
  function _sentDevPool(uint256 _dFeeAmount) private {
    address _dev_address = dev_address;
    _rOwned[_dev_address] = _rOwned[_dev_address].add(_dFeeAmount);
    emit Transfer(owner(), _dev_address, _dFeeAmount);
  }
  ...
  {
    (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 dFee) =
      _getTValues(tAmount);
    (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) =
      _getRValues(tAmount, tFee, tLiquidity, dFee, _getRate());
    return (
      rAmount,
      rTransferAmount,
      rFee,
    ...
    )
  {
    ...
    uint256 dFee = calculateDevFee(tAmount);
    uint256 tLiquidity = calculateLiquidityFee(tAmount);
    uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(dFee);
    return (tTransferAmount, tFee, tLiquidity, dFee);
  }
    ...
    )
  {
    ...
    uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rdFee);
